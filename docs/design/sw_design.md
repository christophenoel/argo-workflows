...
1. Data Inputs as Artefacts: In a WorkflowTemplate, input data should be provided as an artifact, which is submitted as an argument by the workflow calling this template.
2. Intermediate Artefacts: In a workflow template with multiple steps, intermediate artifacts are stored in the default repository (fast storage).
3. Creating Artefacts: Workflows (which can call the templates) can create artifacts from:
A key in the default S3 repository
A key in a specified internal repository
Remote sources such as S3 or HTTP, offering flexibility.
Credential Agnosticism: WorkflowTemplates do not handle credentials. External artifacts (HTTP, S3) are managed by a workflow generated by the Processing Gateway.
